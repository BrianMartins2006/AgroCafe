...
const LavouraProfilePage = () => {
  ...
  useEffect(() => {
    ...
    fetch((import.meta.env.VITE_API_URL || 'https://agrocafe-backend.onrender.com') + `/api/v1/lavouras/${id}`)
      .then(res => res.json())
      .then(data => setLavoura(data))
      .catch(err => console.error("Erro ao buscar lavoura:", err));

    // Buscar mídia da lavoura
    fetch((import.meta.env.VITE_API_URL || 'https://agrocafe-backend.onrender.com') + `/api/v1/lavouras/${id}/media`)
      .then(res => res.json())
      .then(data => {
        setMedia(data);
        setLoading(false);
